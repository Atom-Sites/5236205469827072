...
function SecondaryService(props) {
    ...
    return (
        <div id={makeId(Anchor)} className="mx-auto max-w-2xl lg:mx-0">
            <img
                className="h-[250px] w-[800px] rounded-xl object-cover shadow-xl ring-neutral-400/10"
                src={ImageUrl}
                alt={AltText}
                loading="lazy"
            />
            <h2 className="mt-8 text-base font-bold uppercase leading-6 tracking-wide text-primary-700">
                {Service}
            </h2>
            <h2 className="mt-2 font-display text-2xl font-bold tracking-tight text-neutral-800">
                {Header}
            </h2>
            <p className="mt-6 text-base leading-7 text-neutral-800">{Body}</p>
        </div>
    )
}
