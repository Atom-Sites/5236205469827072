import { Disclosure } from '@headlessui/react'
import {
    AdjustmentsHorizontalIcon,
    BackspaceIcon,
    CloudArrowUpIcon,
    Cog6ToothIcon,
    PlusIcon,
} from '@heroicons/react/20/solid'
import { TrashIcon } from '@heroicons/react/24/outline'
import { usePathname } from 'next/navigation.js'
import { useState } from 'react'
import { AddPage } from './AddPage.jsx'
import ComponentEditor from './ComponentEditor.jsx'
import { ConfirmationModal } from './ConfirmationModal.jsx'
import { useEditorContext } from './EditorContext.jsx'
import { SiteSettingsEditor } from './SiteSettingsEditor.jsx'
import { UpdateEditor } from './UpdateEditor.jsx'

export default function GlobalEditor() {
    const editor = useEditorContext()

    const { company_id } = editor.global.data

    const [isDeletePageOpen, setIsDeletePageOpen] = useState(false)

    let pagename = usePathname().replace('/', '')
    if (pagename === '') {
        pagename = 'index'
    }

    return (
        <>
            <Disclosure as="nav" className="fixed z-50 w-full bg-white shadow">
                <div className="mx-auto h-16 px-4">
                    <div className="flex h-full justify-end gap-2 align-middle">
                        <div className="mr-auto flex shrink-0 items-center">
                            <UpdateEditor company_id={company_id} />
                        </div>

                        <div
                            className={`my-4 mr-2 flex shrink-0 items-center gap-4 border-r-2 border-r-neutral-300 pr-4 ${
                                editor.has_updates ? 'visible' : 'invisible'
                            }`}
                        >
                            <button
                                type="button"
                                className="focus-visible:transparent relative inline-flex items-center gap-x-1.5 rounded-full bg-neutral-200 px-2 py-2 text-sm font-semibold text-blue-950 shadow-sm transition-all ease-in hover:bg-red-400 hover:text-neutral-950 focus-visible:outline focus-visible:outline-0 focus-visible:outline-offset-2"
                                onClick={editor.revert}
                                title="Revert changes"
                            >
                                <BackspaceIcon
                                    className="h-5 w-5"
                                    aria-hidden="true"
                                />
                            </button>

                            <button
                                type="button"
                                className="focus-visible:transparent relative inline-flex items-center gap-x-1.5 rounded-full bg-neutral-200 px-2 py-2 text-sm font-semibold text-blue-950 shadow-sm transition-all ease-in hover:bg-green-400 focus-visible:outline focus-visible:outline-0 focus-visible:outline-offset-2"
                                onClick={editor.save}
                                title="Save changes"
                            >
                                {editor.is_saving ? (
                                    <svg
                                        aria-hidden="true"
                                        className="inline h-5 w-5 animate-spin fill-blue-600 text-gray-200 dark:text-gray-600"
                                        viewBox="0 0 100 101"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                                        <path
                                            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                                            fill="currentColor"
                                        />
                                        <path
                                            d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                                            fill="currentFill"
                                        />
                                    </svg>
                                ) : (
                                    <CloudArrowUpIcon
                                        className="h-5 w-5"
                                        aria-hidden="true"
                                    />
                                )}
                            </button>
                        </div>

                        {pagename !== 'index' && (
                            <div className="mr-2 flex shrink-0 items-center">
                                <button
                                    type="button"
                                    className="focus-visible:transparent relative inline-flex items-center gap-x-1.5 rounded-full bg-neutral-200 px-2 py-2 text-sm font-semibold text-blue-950 shadow-sm transition-all ease-in hover:bg-red-400 focus-visible:outline focus-visible:outline-0 focus-visible:outline-offset-2"
                                    title="Delete Page"
                                    onClick={() => {
                                        setIsDeletePageOpen(true)
                                    }}
                                >
                                    <TrashIcon
                                        className="h-5 w-5"
                                        aria-hidden="true"
                                    />
                                </button>
                            </div>
                        )}

                        <div className="flex shrink-0 items-center">
                            <button
                                type="button"
                                className="focus-visible:transparent relative inline-flex items-center gap-x-1.5 rounded-full bg-neutral-200 px-2 py-2 text-sm font-semibold text-blue-950 shadow-sm transition-all ease-in hover:bg-green-400 focus-visible:outline focus-visible:outline-0 focus-visible:outline-offset-2"
                                title="Add Page"
                                onClick={() => {
                                    editor.page.visible({ mode: 'add' })
                                        ? editor.page.close()
                                        : editor.page.open({ mode: 'add' })
                                }}
                            >
                                <PlusIcon
                                    className="h-5 w-5"
                                    aria-hidden="true"
                                />
                            </button>
                        </div>

                        <div className="flex shrink-0 items-center">
                            <button
                                type="button"
                                className="focus-visible:transparent relative ml-2 inline-flex items-center gap-x-1.5 rounded-full bg-blue-500 px-2 py-2 text-sm font-semibold text-white shadow-sm transition-all ease-in hover:bg-blue-400 focus-visible:outline focus-visible:outline-0 focus-visible:outline-offset-2"
                                title="Edit Page Settings"
                                onClick={() => {
                                    editor.page.visible({ mode: 'settings' })
                                        ? editor.page.close()
                                        : editor.page.open({ mode: 'settings' })
                                }}
                            >
                                <AdjustmentsHorizontalIcon
                                    className="h-5 w-5"
                                    aria-hidden="true"
                                />
                            </button>
                        </div>
                        <div className="flex shrink-0 items-center">
                            <button
                                type="button"
                                className="focus-visible:transparent relative ml-2 inline-flex items-center gap-x-1.5 rounded-full bg-blue-500 px-2 py-2 text-sm font-semibold text-white shadow-sm transition-all ease-in hover:bg-blue-400 focus-visible:outline focus-visible:outline-0 focus-visible:outline-offset-2"
                                title="Edit Site Settings"
                                onClick={() => {
                                    editor.global.visible()
                                        ? editor.global.close()
                                        : editor.global.open()
                                }}
                            >
                                <Cog6ToothIcon
                                    className="h-5 w-5"
                                    aria-hidden="true"
                                />
                            </button>
                        </div>

                        <ComponentEditor
                            file="page.json"
                            componentName="page"
                            component_props={editor.page.data}
                            open={editor.page.visible({ mode: 'settings' })}
                            onSave={editor.save}
                            onChange={editor.page.change}
                            onCancel={() => {
                                editor.page.cancel()
                                editor.page.close()
                            }}
                            onClose={editor.page.close}
                        />
                    </div>
                </div>
            </Disclosure>

            <AddPage />

            <SiteSettingsEditor />

            <ConfirmationModal
                variant="danger"
                title="Delete Page"
                message={
                    <>
                        Are you sure you want to delete the
                        <strong> {editor.page.name}</strong> page?
                    </>
                }
                prompt="Delete"
                open={isDeletePageOpen}
                onClose={() => {
                    setIsDeletePageOpen(false)
                }}
                loading={editor.page.deleting}
                onConfirm={editor.page.delete}
            />

            <div className="h-16" />
        </>
    )
}
